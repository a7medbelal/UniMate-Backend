...
namespace Uni_Mate.Features.StudentManager.ConnectionInfoSave.Command
{
    ...
    public class ConnectionInfoHandler : BaseWithoutRepositoryRequestHandler<ConnectionInfoCommand, RequestResult<bool>,Student>
    {
        public ConnectionInfoHandler(BaseWithoutRepositoryRequestHandlerParameters<Student> parameters) : base(parameters)
        {
        ...
        public override async Task<RequestResult<bool>> Handle(ConnectionInfoCommand request, CancellationToken cancellationToken)
        {
            ...
            var student = await _repositoryIdentity.Get(x => x.Id == userId).FirstOrDefaultAsync();
